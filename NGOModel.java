...
public class NGOModel {
	private static Logger log = Logger.getLogger(NGOModel.class);
	 ...
	 * Next pk for ngo 
	 */
	public Integer nextPKNGO() throws DatabaseException {
		log.debug("Model nextPK Started");
		Connection conn = null;
		int pk = 0;
		...
		try {
			conn = JDBCDataSource.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(ID) FROM NGO");
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				pk = rs.getInt(1);
			}
	 ...
	 * 
	 */
	public long add(UserBean bean, NGOBean ngoBean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt2 = conn.prepareStatement("INSERT INTO NGO VALUES('"+ngoPk+"','"+ngoBean.getNgoName()+"','"+pk+"')");
			pstmt.setInt(1, pk);
			...
			pstmt.setString(10, bean.getUniqueId());		
			...
			pstmt.setString(15, bean.getStatus());
	...
	public UserBean findByLogin(String login) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM USER WHERE LOGINID=?");
		UserBean bean = null;
		Connection conn = null;
	...
	/*
	 * FIND FROM BOTH TABLE USER & NGO
	 */
	public UserBean findFromBothTableByPK(long pk) throws ApplicationException {
		log.debug("Model findByPK Started");
		StringBuffer sql = new StringBuffer("SELECT * FROM USER INNER JOIN NGO ON USER.ID = NGO.USERID WHERE USER.ID=?");
		UserBean bean = null;
		Connection conn = null;
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setModifiedDatetime(rs.getTimestamp(14));
				bean.setStatus(rs.getString(15));
				bean.setNgoName(rs.getString(17));
			}
			rs.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List search(UserBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM USER inner join ngo on user.id = ngo.userid WHERE 1=1");
...
public void accept(UserBean bean) throws ApplicationException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn.prepareStatement("UPDATE USER SET STATUS = '"+1+"' WHERE ID = ? ");
			pstmt.setLong(1, bean.getId());
			pstmt.executeUpdate();
			conn.commit(); // End transaction
			pstmt.close();
	...
	public long registerUser(UserBean bean, NGOBean ngoBean)
			throws ApplicationException, DuplicateRecordException {

		log.debug("Model add Started");

		long pk = add(bean,ngoBean);

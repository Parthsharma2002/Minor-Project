...
public class LoginCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			...
			ServletUtility.forward(OMDSView.LOGIN_VIEW, request, response);
			return;
		}
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(OMDSView.WELCOME_CTL, request, response);
			return;
		}
			...
			try {
				...
				if (bean != null) {					
					...
					if(bean.getStatus().equals("0")) {
						...
						ServletUtility.setErrorMessage("Wait fot the Admin to confirm registration", request);
						ServletUtility.forward(getView(), request, response);
						return;
